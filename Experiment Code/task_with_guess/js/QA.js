$(document).ready(function(){
	...
	/*$("#article-no").click(function (event) {
		...
		if(guess &&(!$.isNumeric($("#guess").attr('value')) || ($.isNumeric($("#guess").attr('value')) && $("#guess").attr('value')<=0))) {
                        alert("Please guess the best positive value. Fractions, commas, letters, or blank answers will not be accepted.");
                   
		}
		else if(!guess && !$("input[name='AnswerChoice']:checked").val()) { 
		...
		else
		{
		    ...
		    else{//choice of answers
	 		    // save clicked 
			    var AnsClicked = $("input[name='AnswerChoice']:checked").val();
			    var AnsClickedStr = 'AnswerChoice=' + AnsClicked

			    AjaxDataStr = AnsClickedStr;
