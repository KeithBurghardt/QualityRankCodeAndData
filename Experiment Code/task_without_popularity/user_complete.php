...
//if(validate_captcha()) {
  if(1){  
    ...
    for ($i=0; $i < /*count($_SESSION['answers']['NumAns'])*/count($_SESSION['user']['QuestionOrder']); ++$i){
    	$QNum = $i+1;
    	$NumAns = $_SESSION['answers']['NumAns'][$i];
    	$TimeStarted = $_SESSION['answers']['TimeStarted'][$i];
	$TimeStarted = date('YmdHis',$TimeStarted);
	$RandQ = (int) $_SESSION['user']['QuestionOrder'][$i];
	$int_guess = (int)$guess;

	if($guess){
		$AnsGuess = (float) $_SESSION['answers']['guess'][$i];
	        // $guess: whether answer is guessed on (True/False()
    	        // $AnsGuess: value of answer (reuses "guess" column)
		//var_dump($int_guess);
                $strsql="INSERT INTO $tb (QNum,NumAns,TimeStarted,survey_code,RandQ,GuessAnswers,Guess)
			VALUES('$QNum','$NumAns','$TimeStarted','$survey_code','$RandQ','$int_guess','$AnsGuess')";
                $result = $conn->query($strsql);
                if(!$result){
                    echo "An error has occured when inserting a Q&A record to the table\n";
                    echo "Error:".$conn->errno. "  ".$conn->error;
	    ...
	    $AnsGuesses = $_SESSION['answers']['AnsVals'][$RandQ-1];
	    $AnsOrder = $_SESSION['answers']['AnsOrder'][$i];
	    // $guess: whether answer is guessed on (True/False()
	    // $AnsGuess: value of answer (reuses "guess" column)
	
	    /*foreach($AnsGuesses as $AnsGuess){
                $strsql="INSERT INTO $tb (QNum,NumAns,TimeStarted,survey_code,RandQ,GuessAnswers,Guess,AnswerChosen)
			VALUES('$QNum','$NumAns','$TimeStarted','$survey_code','$RandQ','$int_guess','$AnsGuess','$AnswerChosen')";
                $result = $conn->query($strsql);
                if(!$result){
                    echo "An error has occured when inserting a Q&A record to the table\n";
                    echo "Error:".$conn->errno. "  ".$conn->error;
	    ...
	    for ($j=0; $j < $NumAns; ++$j){
	        $Answer = $AnsOrder[$j]; 
	        $Score = $popularity[$j];
		$AnsGuess = $AnsGuesses[$j];
		$strsql="INSERT INTO $tb (QNum,NumAns,TimeStarted,survey_code,RandQ,GuessAnswers,Guess,AnswerChosen,AnswerOrder,Score)
                        VALUES('$QNum','$NumAns','$TimeStarted','$survey_code','$RandQ','$int_guess','$AnsGuess','$AnswerChosen','$Answer','$Score')";

		//var_dump($strsql);
	        //$strsql="INSERT INTO $tb (QNum,survey_code) VALUES('$QNum','$survey_code')";
		$result = $conn->query($strsql);
                if(!$result){
                    echo "An error has occured when inserting a Q&A record to the table\n";
                    echo "Error:".$conn->errno. "  ".$conn->error;
