$(document).ready(function(){
	...
	/*$("#article-no").click(function (event) {
		...
		// if no input in guess
		// or if guess is not a number
		//$("#msgbox").removeClass().addClass('messagebox').text('Loading Data...').fadeIn(100);
		if(!$.isNumeric($("input[type=text][name=guess]"))) { 
			alert("Please guess the best value."); 
		}
		else
		{
		    var AjaxDataStr = "";
			...
			AjaxDataStr += "&" + MouseHoverIn + "&" + MouseHoverOut;
			    ...
			    elseif(msg == "Answer is not valid")
			    {
					  $(this).html('Reloading page...').addClass('messageboxok').fadeTo(100,1,
                                          function()
                                          {
